...
/*!
 * \brief implement the logic of vblda7q_rgb565 defined in aci_mve_lib.h
 */
...
void arm_mix_pack_blue_uint16x2(uint16_t * out, const uint16_t * in1, bool is_bottom)
{
    int32_t         blkCnt;
    blkCnt = 2;
    if (is_bottom) {
        do {
            uint8_t B = ((uint8_t *)(in1++))[0] >> 3;
...
void arm_mix_pack_green_uint16x2(uint16_t * out, const uint16_t * in1, bool is_bottom)
{
    int32_t         blkCnt;
    blkCnt = 2;
    if (is_bottom) {
        do {
            uint8_t G = ((uint8_t *)(in1++))[0] >> 2;
...
void arm_mix_pack_red_uint16x2(uint16_t * out, const uint16_t * in1, bool is_bottom)
{
    int32_t         blkCnt;
    blkCnt = 2;
    if (is_bottom) {
        do {
            uint8_t R = ((uint8_t *)(in1++))[0] >> 3;
...
ACI_Status arm_2d_rgb565_aci_fvp::exec_vcx2_beatwise(const ACIVCX2DecodeInfo * decode_info,
                                                     ...
                                                     uint32_t * result)
{
    /*! \note We valid the Pn, `imm`, and ignore the `accumulate` in this implementation
     *        How to interpret and use imm is IMPLEMENTATION DEFINED. For example,
    ...
    switch (decode_info->imm & 0x3f) {
      ...
      case 0b0000100:
          ...
          *result = *(uint32_t *) & d_val;
          return ACI_STATUS_OK;
          break;
      case 0b0001000:   /* pack red */
          arm_mix_pack_red_uint16x2((uint16_t *) & d_val, (uint16_t *) & m_val, is_bottom);
          *result = *(uint32_t *) & d_val;
          return ACI_STATUS_OK;
          break;
      case 0b0010000:   /* pack green */
          arm_mix_pack_green_uint16x2((uint16_t *) & d_val, (uint16_t *) & m_val, is_bottom);
          *result = *(uint32_t *) & d_val;
          return ACI_STATUS_OK;
          break;
      case 0b0100000:   /* pack blue */
          arm_mix_pack_blue_uint16x2((uint16_t *) & d_val, (uint16_t *) & m_val, is_bottom);
          *result = *(uint32_t *) & d_val;
          return ACI_STATUS_OK;
          break;
      default:
...
ACI_Status arm_2d_rgb565_aci_fvp::exec_vcx3_beatwise(const ACIVCX3DecodeInfo * decode_info,
                                                     ...
                                                     uint32_t * result)
{
    /*! \note We valid the Pn, `accumulate` and `imm` in this implementation
     *        How to interpret and use imm and accumulate is IMPLEMENTATION DEFINED. 
     *        For example:
     *        use can use imm as a parameter or a way to address different instructions
     *        or instruction variants.
     */
